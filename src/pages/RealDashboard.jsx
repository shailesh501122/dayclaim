 ...
 * using premium DayClaim components and real backend APIs.
 */
export function RealDashboard() {
  ...
  const inventoryChartData = inventory?.byBucket
    ? Object.entries(inventory.byBucket).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
  ...
  const assignmentChartData = assignment?.byUser
    ? Object.entries(assignment.byUser).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
    ...
    <section className="section real-screen">
      <div className="module-hero">
        <div className="hero-left">
          <span className="module-kicker">
            <LayoutDashboard size={12} /> Analytics Dashboard
          </span>
          <h1>System Overview</h1>
          <p>
            Real-time visibility into claim inventory, allocations, and rule
            engine performance for the selected business unit.
          </p>
        </div>
      ...
      {(orgsError || error) && (
        <div className="module-error" style={{ margin: "14px 0" }}>
          ...
          <p>{orgsError || error}</p>
          <button className="ghost-button sm" onClick={load}>
            Retry Connection
          </button>
        </div>
      )}
      ...
      {orgsLoading || (loading && !inventory) ? (
        <div className="module-loading">
          ...
          />
          <p>
            Retrieving real-time operational data from the DayClaim backend...
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-4 section">
            ...
            <StatCard
              ...
              value={
                ruleEngine?.lastRunClaimsProcessed?.toLocaleString() ?? "0"
              }
              delta={
                ruleEngine?.lastRunAtUtc
                  ? new Date(ruleEngine.lastRunAtUtc).toLocaleDateString()
                  : "N/A"
              }
              tone="indigo"
          ...
          <div className="grid grid-2 section">
            <DonutCard
              title="Inventory Distribution (Buckets)"
          ...
          {ruleEngine?.lastRunAtUtc && (
            <div className="section">
              <HorizontalBars
                ...
                data={Object.entries(ruleEngine.byBucket).map(
